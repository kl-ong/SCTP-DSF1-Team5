...
FROM {{ ref('dim_orders_analysis') }} AS o

	JOIN {{ ref('fact_order_items_stg') }} AS oi
	    ON o.order_id = oi.order_id
	LEFT JOIN {{ ref('fact_order_payments_analysis') }} AS op
	    ON o.order_id = op.order_id
	LEFT JOIN {{ ref('dim_products_analysis') }} AS p
	    ON oi.product_id = p.product_id
	LEFT JOIN {{ ref('dim_product_category_name_translation_stg') }} AS t
    	ON p.product_category_name = t.product_category_name
	LEFT JOIN {{ ref('dim_sellers_analysis') }} AS s
